// Separate from MOCK_MODE (which controls BasePulse scoring). This lets you
// use live trust scores while still simulating wallet execution, since real
// CDP wallet wiring isn't built yet. Defaults to true (safe) unless
// explicitly set to "false" once executeReal() is actually wired up.
const WALLET_MOCK_MODE = process.env.WALLET_MOCK_MODE !== "false";
  ...
  if (action === "approve") {
    if (WALLET_MOCK_MODE) {
      return log({
        ...
        reason,
        executed: true,
        note: "WALLET_MOCK_MODE — simulated execution, no real transaction sent."
      });
    }
...
async function executeReal({ address, amountUsd, reason }) {
  ...
  throw new Error("executeReal() not yet wired to live CDP wallet — set WALLET_MOCK_MODE=true (the default) for now.");
}
